...
setlog(Goal,OutConstrLst,Res) :-
    set_default,
    catch(setlog1(Goal,OutConstrLst,Res),Msg,excpt_action(Msg)).
...
setlog(Goal,TimeOut,OutConstrLst,Res,Options) :-
    set_default,
    catch(setlog1(Goal,TimeOut,OutConstrLst,Res,Options),Msg,excpt_action(Msg)).

setlog_no_catch(Goal,TimeOut,OutConstrLst,Res,Options) :-  % for internal use only
    set_default,
...
setlog_str(GoalString,VarNames_or_Eqs,OutConstr,Res) :- 
  set_default,
  catch(setlog1_str(GoalString,VarNames_or_Eqs,OutConstr,Res),Msg,excpt_action(Msg)).
...
setlog_str(GoalString,VarNames_or_Eqs,TimeOut,OutConstr,Res,Options) :-
    set_default,
    catch(setlog1_str(GoalString,VarNames_or_Eqs,TimeOut,OutConstr,Res,Options),Msg,excpt_action(Msg)).
...
negate(A,NegA) :-
  nonvar(A),
  functor(A,F,N), functor(A1,F,N),       % A1 is structural equivalent to A but with variable args
  findall(_,isetlog((A1 :- _),_),[_]),   % only one clause defines A  
  isetlog((A1 :- B),_),
  A1 =.. [_|Params], 
  forall(member(P,Params),var(P)),       % all A's arguments in its defining clause are variables  
  term_variables(A1,VarA),
...
arithm_expr_error_msg(time_out(_)) :- !,
    throw(time_out).
...
%  [subset_unify,comp_fe]). 
%  [un_fe,oplus_fe]). %£££££££££
%  [ran_fe,noirules,strategy(ordered)]). %£££££££££
